...
use Strawberry\HelloBundle\Model\UserPeer;
use Strawberry\HelloBundle\Model\UserQuery;
 ...
 * @method UserQuery orderByTeamId($order = Criteria::ASC) Order by the team_id column
 *
 ...
 * @method UserQuery groupByTeamId() Group by the team_id column
 *
 * @method UserQuery leftJoin($relation) Adds a LEFT JOIN clause to the query
 * @method UserQuery rightJoin($relation) Adds a RIGHT JOIN clause to the query
 * @method UserQuery innerJoin($relation) Adds a INNER JOIN clause to the query
 *
 * @method UserQuery leftJoinTeam($relationAlias = null) Adds a LEFT JOIN clause to the query using the Team relation
 * @method UserQuery rightJoinTeam($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Team relation
 * @method UserQuery innerJoinTeam($relationAlias = null) Adds a INNER JOIN clause to the query using the Team relation
 *
 ...
 * @method User findOneByTeamId(int $team_id) Return the first User filtered by the team_id column
 *
 ...
 * @method array findByTeamId(int $team_id) Return User objects filtered by the team_id column
 */
abstract class BaseUserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'Strawberry\\HelloBundle\\Model\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name`, `gender`, `city`, `active`, `team_id` FROM `user` WHERE `id` = :p0';
        try {
     ...
     * $query->filterByTeamId(array('max' => 12)); // WHERE team_id <= 12
     * </code>
     *
     * @see       filterByTeam()
     *
    ...
    public function filterByTeamId($teamId = null, $comparison = null)
    {
        if (is_array($teamId)) {
            $useMinMax = false;
            if (isset($teamId['min'])) {
                $this->addUsingAlias(UserPeer::TEAM_ID, $teamId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($teamId['max'])) {
                $this->addUsingAlias(UserPeer::TEAM_ID, $teamId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserPeer::TEAM_ID, $teamId, $comparison);
    }

    /**
     * Filter the query by a related Team object
     *
     * @param   Team|PropelObjectCollection $team The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 UserQuery The current query, for fluid interface
    ...
    public function filterByTeam($team, $comparison = null)
    {
        if ($team instanceof Team) {
            return $this
                ->addUsingAlias(UserPeer::TEAM_ID, $team->getId(), $comparison);
        } elseif ($team instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(UserPeer::TEAM_ID, $team->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Strawberry\HelloBundle\Model\TeamQuery A secondary query class using the current class as primary query
     */
    public function useTeamQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinTeam($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Team', '\Strawberry\HelloBundle\Model\TeamQuery');
    }
